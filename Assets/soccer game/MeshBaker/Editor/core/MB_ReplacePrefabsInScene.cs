    ...
    public class MB_ReplacePrefabsInScene : MonoBehaviour
    {
        ...
        public int ReplacePrefabInstancesInScene(GameObject mySrcPrefab, GameObject myTargPrefab, List<Error> objsWithErrors)
        {
            ...
            if (replaceEnforceStructure)
            {
                ...
                if (!structureIsSame)
                {
                    Debug.LogError("Prefab Structure is not the same for prefabs:" + mySrcPrefab + " and " + myTargPrefab);
                    return 0;
                }
            }
            ...
            Dictionary<Component, Component2MeshAndMaterials> component2MeshAndMats)
        {
            ...
            for (int i = 0; i < srcComponents.Length; i++)
            {
                ...
                if (src2targetObjMap != null && component2MeshAndMats != null)
                {
                    ...
                    CollectAssetReferncesForComponent(targComponents[i], component2MeshAndMats);
                }
            }
        ...
        private static void CollectAssetReferncesForComponent(Component comp, Dictionary<Component, Component2MeshAndMaterials> propRefereces)
        {
            ...
            Dictionary<Component, Component2MeshAndMaterials> component2MeshAndMats)
        {
            ...
            for (int i = 0; i < srcComps.Length; i++)
            {
                if (srcComps[i].GetType() == targComps[i].GetType())
                {
                    ...
                    {
                        SerializedProperty sp = serializedObject.FindProperty(targetInernalRefs[refIdx].propertyName);
                        sp.objectReferenceValue = targetInernalRefs[refIdx].obj;
                        //Debug.Log("Comp" + targComps[i] + "  RestoringProp: " + targetInernalRefs[refIdx].propertyName + "  val " + targetInernalRefs[refIdx].obj);
                    }
                    ...
                    if (component2MeshAndMats.ContainsKey(targComps[i]))
                    {
                        ...
                        {
                            prop = targSO.FindProperty(meshAndMats.props[prpIdx].propertyName);
